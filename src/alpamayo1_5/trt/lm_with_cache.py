...
from alpamayo1_5.trt.prefix_cache import (
    PrefixKVCache,
...
class Qwen3VLTextModelWithCacheWrapper(nn.Module):
    ...
    def _build_causal_mask(
        ...
        base = torch.cat(
            [torch.zeros(q_len, prefix_len, device=device, dtype=torch.float32), causal_q],
            dim=-1,
        )
    ...
    def forward(
        ...
        if isinstance(past_key_values, PrefixKVCache):
            ...
            cache = PrefixKVCache(
                _maybe_to(past_key_values[0], device=inputs_embeds.device, dtype=inputs_embeds.dtype),
                _maybe_to(past_key_values[1], device=inputs_embeds.device, dtype=inputs_embeds.dtype),
            )
        else:
        ...
        if cache_position is None:
            ...
            cache_position = torch.arange(prefix_len, prefix_len + inputs_embeds.shape[1], device=inputs_embeds.device)
...
class Qwen3VLTextModelPrefillWrapper(nn.Module):
    ...
    @staticmethod
    def _apply_dense_deepstack(hidden_states: torch.Tensor, deepstack_layer_embeds: torch.Tensor | None) -> torch.Tensor:
    ...
    def forward(
        ...
        q_len = inputs_embeds.shape[1]
        if attention_mask is not None and attention_mask.ndim == 4 and attention_mask.shape[-2] == q_len:
        ...
        if isinstance(deepstack_visual_embeds, (list, tuple)) and len(deepstack_visual_embeds) > 0:
            deepstack_visual_embeds = torch.stack(
                [d.to(device=inputs_embeds.device, dtype=inputs_embeds.dtype) for d in deepstack_visual_embeds], dim=0
            )
        ...
        for layer_idx, decoder_layer in enumerate(language_model.layers):
            ...
            if isinstance(deepstack_visual_embeds, torch.Tensor):
                ...
                elif deepstack_visual_embeds.ndim == 3 and visual_pos_masks is not None and layer_idx < deepstack_visual_embeds.shape[0]:
...
class Qwen3VLPrefillRuntimeAdapter(nn.Module):
    ...
    def forward(
        ...
        if visual_pos_masks is None:
            visual_pos_masks = torch.zeros(bsz, q_len, dtype=torch.bool, device=inputs_embeds.device)
        else:
        ...
        if isinstance(deepstack_visual_embeds, (list, tuple)):
            ...
            if deepstack_visual_embeds.ndim == 4:
                dense_deepstack = deepstack_visual_embeds.to(device=inputs_embeds.device, dtype=inputs_embeds.dtype)
            elif deepstack_visual_embeds.ndim == 3:
                for idx in range(min(self.num_deepstack, deepstack_visual_embeds.shape[0])):
                    layer_embeds = deepstack_visual_embeds[idx].to(device=inputs_embeds.device, dtype=inputs_embeds.dtype)
                    if layer_embeds.numel() > 0:
                        dense_deepstack[idx] = dense_deepstack[idx].masked_scatter(mask3d, layer_embeds.reshape(-1))
...
def _export_wrapper(
    ...
    dynamic_shapes = {
        ...
        "past_key_values": ({1: batch_dim, 3: prefix_dim}, {1: batch_dim, 3: prefix_dim}),
    }
    ...
    def _wrapper_forward(
        ...
        **kwargs,
        ...
        if visual_pos_masks is not None or deepstack_visual_embeds is not None:
            ...
            inputs_embeds = _maybe_to(inputs_embeds, device=self._wrapper_device, dtype=self._wrapper_dtype)
            ...
            if isinstance(attention_mask, torch.Tensor) and attention_mask.ndim == 2:
                ...
                if cur_len > q_len:
                    ...
                    pad = torch.ones(bsz, q_len - cur_len, dtype=prefill_attention_mask.dtype, device=self._wrapper_device)
            ...
            if visual_pos_masks is None:
                visual_pos_masks = torch.zeros(bsz, q_len, dtype=torch.bool, device=self._wrapper_device)
            else:
            ...
            if not isinstance(wrapper_out, (tuple, list)) or len(wrapper_out) != 3:
                raise ValueError("Prefill wrapper expected 3 outputs (hidden_states, updated_k, updated_v)")
            hidden_states, updated_k, updated_v = wrapper_out
            updated_prefix_cache = PrefixKVCache(updated_k, updated_v)
            return BaseModelOutputWithPast(last_hidden_state=hidden_states, past_key_values=updated_prefix_cache)
        ...
        if isinstance(attention_mask, torch.Tensor) and attention_mask.ndim == 2:
            ...
            if cur_len > target_len:
                ...
                pad = torch.ones(bsz, target_len - cur_len, dtype=wrapper_attention_mask.dtype, device=self._wrapper_device)
        ...
        if cache_position is None:
            prefix_len = prefix_k.shape[3]
            cache_position = torch.arange(prefix_len, prefix_len + q_len, device=self._wrapper_device, dtype=torch.long)
        elif cache_position.ndim == 0:
            cache_position = cache_position.view(1)
        if position_ids is None:
            position_ids = cache_position.view(1, 1, -1).expand(3, bsz, -1).clone()
        ...
        if len(wrapper_out) == 3:
            ...
            raise ValueError("Compiled wrapper expected 3 outputs (hidden_states, updated_k, updated_v)")
        return BaseModelOutputWithPast(last_hidden_state=hidden_states, past_key_values=updated_prefix_cache)
...
def compile_vlm_lm_trt_with_cache(
    ...
    example_prefix_k = torch.zeros(num_layers, bsz, num_kv_heads, opt_prefix_len, head_dim, dtype=dtype, device=device)
    example_prefix_v = torch.zeros_like(example_prefix_k)
    example_past_key_values = (example_prefix_k, example_prefix_v)
    example_attention_mask = torch.ones(bsz, opt_prefix_len + opt_seq_len, dtype=torch.long, device=device)
    ...
    with torch_tensorrt.dynamo.Debugger() if debug else nullcontext():
        trt_backbone = torch_tensorrt.dynamo.compile(ep, inputs=trt_inputs, **trt_settings)
    ...
    example_prefill_attention_mask = torch.ones(bsz, opt_seq_len, dtype=torch.long, device=device)
    example_prefill_visual_pos_masks = torch.zeros(bsz, opt_seq_len, dtype=torch.bool, device=device)
    example_prefill_visual_pos_masks[:, :opt_visual_tokens] = True
    if num_deepstack > 0:
        example_prefill_deepstack_visual_embeds = torch.zeros(
            num_deepstack, bsz, opt_seq_len, hidden_size, dtype=dtype, device=device
        )
        example_prefill_deepstack_visual_embeds[:, :, :opt_visual_tokens, :] = torch.randn(
            num_deepstack, bsz, opt_visual_tokens, hidden_size, dtype=dtype, device=device
        )
    else:
        example_prefill_deepstack_visual_embeds = torch.zeros(
            0, bsz, opt_seq_len, hidden_size, dtype=dtype, device=device
        )
    ...
    short_prefix_k = torch.zeros(num_layers, 1, num_kv_heads, check_prefix_len, head_dim, dtype=dtype, device=device)
    short_prefix_v = torch.zeros_like(short_prefix_k)
    try:
        with torch.no_grad():
            trt_out_raw = trt_backbone(short_attn, short_pos, short_embeds, (short_prefix_k, short_prefix_v))
            if accuracy_check:
                ref_h = wrapper(
    ...
    prefill_runtime_adapter = Qwen3VLPrefillRuntimeAdapter(trt_prefill, num_deepstack=num_deepstack).eval()
